...
struct EditableOwner: Identifiable, Codable, Hashable {
    ...
    var authorMappingEntry: AuthorMappingEntry {
        AuthorMappingEntry(
            email: email,
            displayName: displayName.isEmpty ? email : displayName,
            github: githubUsername.isEmpty ? nil : githubUsername
        )
    }

    init(email: String, displayName: String, githubUsername: String) {
        self.email = email
        self.displayName = displayName
        self.githubUsername = githubUsername
    }

    init(_ entry: AuthorMappingEntry) {
        self.email = entry.email
        self.displayName = entry.displayName
        self.githubUsername = entry.github ?? ""
    }
}
...
final class ScanModel {
    ...
    private func exportAuthorsYAML(to url: URL) throws {
        try ensureParentDirectoryExists(for: url)

        let header = """
        # Auto-generated by Stella. UI에서 담당자 정보를 수정하면 이 파일이 갱신됩니다.

        """
        let entries = manualOwners.map(\.authorMappingEntry)
        try (header + AuthorMapper.serialize(entries)).write(to: url, atomically: true, encoding: .utf8)
    }
    ...
    func loadOwnerYAMLFiles() {
        loadManualOwnersFromAuthorsYAML()
        loadAssignmentsFromOwnersYAML()
        persistDefaults()
    }

    private func loadManualOwnersFromAuthorsYAML() {
        guard let url = existingFileURL(authorsPath),
              let mapper = try? AuthorMapper.load(from: url)
        else { return }
        manualOwners = mapper.entries.map(EditableOwner.init)
            .sorted { $0.authorRef.displayName < $1.authorRef.displayName }
    }

    private func loadAssignmentsFromOwnersYAML() {
        guard let url = existingFileURL(ownersPath),
              let owners = try? OwnersLoader.load(from: url)
        else { return }
        ownerAssignments = Dictionary(
            uniqueKeysWithValues: owners.endpointOwners.map { key, email in
                ("\(key.method.rawValue) \(key.path)", email)
            }
        )
    }
    ...
    private func saveOwnerYAMLFiles(authors: Bool, owners: Bool) {
        do {
            if authors, !authorsPath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                try exportAuthorsYAML(to: absoluteURL(authorsPath))
            }
            if owners, !ownersPath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                try exportOwnersYAML(to: absoluteURL(ownersPath))
            }
        } catch {
            statusMessage = "담당자 yml 저장 실패: \((error as NSError).localizedDescription)"
        }
    }
    ...
    func upsertOwner(_ owner: EditableOwner) {
        ...
        persistDefaults()
        saveOwnerYAMLFiles(authors: true, owners: false)
        if let snapshot {
    ...
    func deleteOwner(_ owner: EditableOwner) {
        ...
        persistDefaults()
        saveOwnerYAMLFiles(authors: true, owners: true)
        if let snapshot {
            self.snapshot = applyingManualOwners(to: snapshot)
        }
    }
    ...
    func assignOwner(email: String?, to entry: CoverageSnapshot.EndpointEntry) {
        ...
        persistDefaults()
        saveOwnerYAMLFiles(authors: false, owners: true)
        if let snapshot {
            self.snapshot = applyingManualOwners(to: snapshot)
        }
    }
